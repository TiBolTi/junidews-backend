            ...
            @if($decoding != null)
                ...
                <div class="card">
                    ...
                    <div class="card-footer bg-transparent detail-text d-none">
                        <p class="fs-5 fw-bold">{{$airline = $decoding['departure_airport']}} --- {{$airline = $decoding['arrival_airport']}}
                        </p>
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <sapn>{{$airline = $decoding['departure_date']}} - {{$airline = $decoding['week_day']}}</sapn>
                            </div>

                            <div class="d-flex text-end flex-column">
                            <p>{{$airline = $decoding['departure_airport']}} <span class="fw-bold"> - Аэропорт</span></p>
                                <div class="d-flex text-end flex-column">
                            <span>{{$airline = $decoding['departure_country']}} <span class="fw-bold"> - Страна</span></span>
                            @if($airline = $decoding['departure_state'] != null)<span>{{$airline = $decoding['departure_state']}} <span class="fw-bold"> - Штат</span>@endif
                                </div>
                            </div>
                        </div>
                        <hr>
                        ...
                        <span>{{$airline = $decoding['arrival_date']}} - {{$airline = $decoding['week_day']}}</span>
                            </div>

                            <div class="d-flex text-end flex-column">
                        <p>{{$airline = $decoding['arrival_airport']}} <span class="fw-bold"> - Аэропорт</span></p>
                                <div class="d-flex text-end flex-column">
                        <span>{{$airline = $decoding['arrival_country']}} <span class="fw-bold"> - Страна</span></span>
                            @if($airline = $decoding['arrival_state'] != null)<span>{{$airline = $decoding['arrival_state']}} <span class="fw-bold"> - Штат</span></span>@endif
                                </div>
                            </div>
                        </div>
                        <hr>
                    <div class="d-flex justify-content-between">
                        <div>
                        <p><span class="fw-bold"> Класс бронирования:</span> {{$airline = $decoding['class_booking']}}</p>
                        <p><span class="fw-bold"> Статус бронирования:</span> {{$airline = $decoding['booking_status']}}</p>
                        <p><span class="fw-bold"> Зарезервированно мест:</span> {{$airline = $decoding['reserved_seats']}} мест</p>
                        </div>
                    <div>
                        <p><span class="fw-bold"> Самолёт:</span> {{$airline = $decoding['plane']}}</p>
                        <p><span class="fw-bold"> Осталось мест:</span> {{$airline = $decoding['remaining_seats']}} мест</p>
                        <p><span class="fw-bold"> Номер рейса:</span> {{$airline = $decoding['flight_number']}}</p>
                </div>
                </div>
